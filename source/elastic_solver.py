...
# Anfang ############################################################################    
    def _setup_problem_wave(self, dt):
        ...
        T =2*pi/(omega*tref)
        print("T = {}".format(T))
        print("tref = {}".format(tref))
        print("tend/tref = {}".format(self._tend/tref))
        ...
        #
        u0_Vector2 = dlfn.Expression(("u0max*x[0] - u0max*x[0]*(x[0] > 0.5) + (u0max-u0max*x[0])*(x[0] > 0.5)", "0.0"), degree=2, u0max=u0max)
        init_dis = u0_Vector2
        ...
        self._solutionoldoldold = u0
        self._solutionoldold = u1
        self._solutionold = u2
        
    def solve_wave_n_iteration(self, dt):
        ## n.th Iteration ##
        ...
        dw_ext = (dlfn.dot(self._null_vector, v) * dt**2
                 + dlfn.dot(5 * self._solutionold - 4 * self._solutionoldold + self._solutionoldoldold,v)) * dV
        ...
        self._solutionoldoldold = self._solutionoldold
        self._solutionoldold = self._solutionold
        self._solutionold = self._solution
