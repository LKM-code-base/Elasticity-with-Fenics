...
from elastic_solver import LinearElasticWaveSolver as WaveSolver
import numpy as np
...
class ProblemBaseWave:
    ...
    ## SEBASTIAN, PLEASE TAKE A LOOK, We don't know how to implement
    def _append_xdmf_file(self, current_time=0.0):
        """
        Write the output to an xdmf file. The solution and additional fields
        are output to the file.
        """
...
class LinearElasticProblem(ProblemBase):
    """
    Class to simulate wave propagation using the
    `LinearElasticWaveSolver`.
...
class LinearElasticWaveProblem(ProblemBaseWave):
    """
    ...
    def _get_filename(self, current_time):
        """
        Class method returning a filename.
        """
        # input check
...
# Anfang ############################################################################
    def solve_wave_problem(self):
        ...
        if not hasattr(self, "_linear_elastic_solver"):
            self._linear_elastic_solver = \
                WaveSolver(self._mesh, self._boundary_markers)
        ...
        # time step 1 and 2
        self._linear_elastic_solver._setup_problem_wave(dt)
        ...
        for n in range(self._numsteps-2): # weil 2 Zeitschritte hard gecodet sind
            ...
            # time step
            self._linear_elastic_solver.solve_wave_n_iteration(dt)
            ...
            # write XDMF-files
            self._write_xdmf_file(current_time=t)
            #self._append_xdmf_file(current_time=t)
